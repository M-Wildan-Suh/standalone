<div x-show="add" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
    <div
        class="w-full max-w-[720px] bg-white pb-6 rounded-md flex flex-col gap-4 relative overflow-hidden border-2 border-byolink-1">
        <button @click="add = false" class=" absolute top-6 right-6 w-6 h-6 text-white hover:text-red-500 duration-300">
        ...
        <div class=" pt-6 pb-3 bg-byolink-1 text-white">
            <h2 class=" px-6 text-2xl font-bold">Tambah Youtube</h2>
        </div>
        <form action="{{route('youtube.store')}}" method="POST">
            ...
            <div class=" w-full space-y-4 px-6">
                <x-admin.component.linkinput title="Link Youtube" placeholder="Masukkan link..."
                    :value="old('link')" name="link" link="Url" />
