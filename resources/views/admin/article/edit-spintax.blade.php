<x-admin.article.form head="Edit Article Spintax" title="Admin - Edit Article Spintax" :spintax="route('article.spin', ['id' => $article->id])" :link="route('article.spin', ['id' => $article->id])" :form="route('article.update', ['article' => $article->id])" >
    ...
    <x-admin.component.textareainput title="Sambutan (Opsional)" placeholder="Masukkan Sambutan" :value="old('greet', $article->greet)" name="greet" />
            ...
            <div class=" w-full flex flex-col max-w-full gap-2 text-sm sm:text-base font-medium">
                ...
                <div class="w-full h-52 sm:h-60 flex items-center justify-center">
                    <div class=" aspect-square max-h-full max-w-full rounded-md overflow-hidden shadow-md shadow-black/20 ">
                        <x-admin.component.imageinput title="" placeholder="Masukkan nama/tipe web..." :value="$article->profile ? asset('storage/images/article/profile/'.$article->profile) : null" name="profile" />
                    </div>
                </div>
            </div>
        </div>
            ...
            <script>
                function socialLinkForm() {
                    return {
                        ...
                        newItem: { type: '', url: '' },
                        items: @json($article->social->toArray()),
                        addItem() {
