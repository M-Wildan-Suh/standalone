<x-admin.article.form head="Edit Article Unique" title="Admin - Edit Article Unique" :link="route('business', ['slug' => $articleShow->slug])" :form="route('article-show.update', ['article_show' => $articleShow->id])">
    ...
    <x-admin.component.textareainput title="Sambutan (Opsional)" placeholder="Masukkan Sambutan" :value="old('greet', $articleShow->articles->greet)" name="greet" />
            ...
            <div class=" w-full flex flex-col max-w-full gap-2 text-sm sm:text-base font-medium">
                ...
                <div class="w-full h-52 sm:h-60 flex items-center justify-center">
                    <div class=" aspect-square max-h-full max-w-full rounded-md overflow-hidden shadow-md shadow-black/20 ">
                        <x-admin.component.imageinput title="" placeholder="Masukkan nama/tipe web..." :value="$articleShow->articles->profile ? asset('storage/images/article/profile/'.$articleShow->articles->profile) : null" name="profile" />
                    </div>
                </div>
            </div>
        </div>
            ...
            <script>
                function socialLinkForm() {
                    return {
                        ...
                        newItem: { type: '', url: '' },
                        items: @json($articleShow->articles->social->toArray()),
                        addItem() {
